...
var MultiStorage = function (options) {
  ...
  var stores = [];
  var cacheOnMiss = (options.cacheOnMiss === undefined) ? true : options.cacheOnMiss;
  ...
  for (var i = 0; i < settings.length; i++) {
    try {
      ...
      if (cacheOnMiss) {
        settings[i].settings.includeKey = true;
        settings[i].settings.permute = false;
      }
  ...
  self.get = function (key, cb) {
    ...
    var attempt = function () {
      ...
      stores[current].get(key, function (v) {
        if (v === null) {
          missed.push(stores[current]);
          process.nextTick(function () {
            current++;
            attempt();
          })
        } else if (cacheOnMiss && missed.length) {
          var sem = missed.length*v.length;
          for (var i = 0; i < missed.length; i++) {
            for (var j = 0; j < v.length; j++) {
              missed[i].set(v[j].key, v[j].val, function () {
                process.nextTick(function () {
                  sem--;
                  if (sem == 0) {
                    var vals = v.map(function (x) {
                      return x.val;
                    });
                    cb(vals);
                  }
